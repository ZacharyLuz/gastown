...
export async function startStreamableHTTPServer(
  ...
  const basePort = parseInt(process.env.PORT ?? "3012", 10);
  ...
  if (useTls) {
    ...
    httpsServer.listen(port, () => console.log(`Gas Town Hooks MCP server on ${protocol}://localhost:${port}/mcp`));
    const shutdown = () => { httpsServer.close(() => process.exit(0)); };
    process.on("SIGINT", shutdown);
    process.on("SIGTERM", shutdown);
  } else {
    const httpServer = app.listen(port, () => console.log(`Gas Town Hooks MCP server on ${protocol}://localhost:${port}/mcp`));
